  ...
  <div class="h-screen bg-slate-50 overflow-hidden">
    ...
    <header class="lg:hidden sticky top-0 z-20 bg-white/90 backdrop-blur border-b border-slate-200">
      <div class="px-4 sm:px-6 h-14 flex items-center justify-between">
        <button
          class="h-12 w-12 rounded-xl bg-azul-marino text-white grid place-items-center shadow-sm flex-shrink-0 active:scale-[0.98] transition"
          @click="toggleSidebar"
    ...
    <div v-if="isSidebarOpen" class="lg:hidden fixed inset-0 z-30">
      ...
      <aside class="absolute left-0 top-0 h-full w-56 bg-azul-marino text-white flex flex-col">
        <div class="flex items-center justify-between px-6 py-5 border-b border-white/10">
          <div class="flex items-center gap-3">
            <img src="/images/logo2.png" alt="Logo RUPAL" class="h-14 w-14 object-contain rounded-2xl" />
            <div>
        ...
        <nav class="flex flex-col flex-1 px-4 py-6 text-sm">
          <button
            class="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-white/10 ring-1 ring-white/10 text-white font-medium hover:bg-naranja-oscuro hover:ring-white transition-all duration-200"
          ...
          <button
            class="mt-2 w-full flex items-center gap-3 px-4 py-3 rounded-xl text-slate-200/90 ring-1 ring-transparent hover:bg-naranja-oscuro hover:text-white hover:ring-white transition-all duration-200"
            @click="navigate('/staff/producers')"
          >
          ...
          <button
            class="mt-2 w-full flex items-center gap-3 px-4 py-3 rounded-xl text-slate-200/90 ring-1 ring-transparent transition-all duration-200"
            :class="isAdmin ? 'hover:bg-naranja-oscuro hover:text-white hover:ring-white cursor-pointer' : 'opacity-50 cursor-not-allowed bg-slate-800/50'"
            :disabled="!isAdmin"
    ...
    <div class="flex h-[calc(100vh-3.5rem)] lg:h-screen">
      <!-- Desktop Sidebar -->
      <aside class="hidden lg:flex w-64 shrink-0 h-full flex-col bg-azul-marino text-white overflow-y-auto">
        <div class="px-6 py-6">
          <div class="flex flex-col items-center gap-2">
            <img src="/images/logo2.png" alt="Logo RUPAL" class="h-32 w-32 object-contain rounded-3xl" />
        <p class="text-sm uppercase tracking-[0.25em] text-slate-300/70 font-semibold text-center">
  Sistema Integrado<br>
  <span class="text-base">RUPAL</span>
</p>
          </div>
        </div>
