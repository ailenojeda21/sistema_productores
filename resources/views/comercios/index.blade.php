        ...
        <div class="flex justify-between items-center mb-6">
            ...
            @if($comercios->isEmpty())
                <a href="{{ route('comercios.create') }}"
                   class="px-4 py-2 bg-naranja-oscuro text-white rounded hover:bg-amarillo-claro font-semibold shadow">Nuevo
                    Comercio</a>
            @else
                <a href="{{ route('comercios.edit', $comercios->first()->id) }}"
                   class="px-4 py-2 bg-azul-marino text-white rounded hover:bg-amarillo-claro font-semibold shadow">Editar
                    Comercio</a>
            @endif
        </div>
                ...
                @empty
                    <tr class="empty-row">
                        <td colspan="4" class="px-4 py-6 text-center text-gray-600">
                            <div class="p-2">
                                @include('comercios.partials._table_empty')
                            </div>
                        </td>
                    </tr>
                @endforelse
