            ...
            <form method="POST" action="{{ route('propiedades.update', $propiedad) }}" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <button type="button" id="toggleMap"
                                class="px-4 py-2 bg-azul-marino text-white rounded hover:bg-amarillo-claro hover:text-azul-marino font-semibold shadow transition">
                            Ver mapa
                        </button>
                        ...
                        <div id="mapContainer" class="hidden mt-4">
                            ...
                            <p class="text-sm text-gray-500 mt-2">
                                ...
                                <span id="coordenadas"
                                      class="font-semibold">{{ $propiedad->lat ? $propiedad->lat . ', ' . $propiedad->lng : 'No seleccionadas' }}</span>
                            </p>
                            <p class="text-sm text-gray-500">Haz click en el mapa para ubicar la propiedad. También
                                puedes arrastrar el pin.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input id="hectareas" name="hectareas" type="number" step="0.01"
                               class="w-full p-2 border border-gray-300 rounded"
                               value="{{ old('hectareas', isset($propiedad) ? $propiedad->hectareas : '') }}">
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="flex items-center">
                            <input type="checkbox" name="es_propietario" id="es_propietario"
                                   class="mr-2 custom-checkbox" {{ old('es_propietario', $propiedad->es_propietario) ? 'checked' : '' }}>
                            <label for="es_propietario">¿Es propietario?</label>
                        </div>
                        <div class="flex items-center">
                            <input type="checkbox" name="cierre_perimetral" id="cierre_perimetral"
                                   class="mr-2 custom-checkbox" {{ old('cierre_perimetral', $propiedad->cierre_perimetral) ? 'checked' : '' }}>
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center">
                            <input type="checkbox" name="derecho_riego" id="derecho_riego" class="mr-2 custom-checkbox"
                                   {{ old('derecho_riego', $propiedad->derecho_riego) ? 'checked' : '' }} onchange="document.getElementById('tipoDerechoRiegoDiv').style.display = this.checked ? 'block' : 'none';">
                            <label for="derecho_riego">¿Tiene derecho de riego?</label>
                        </div>
                        <div id="tipoDerechoRiegoDiv" style="display:none;" class="ml-7">
                            <label for="tipo_derecho_riego" class="block text-gray-700 font-semibold mb-1">Tipo de
                                derecho de riego:</label>
                            <select name="tipo_derecho_riego" id="tipo_derecho_riego"
                                    class="w-full p-2 border border-gray-300 rounded">
                                <option value="">Seleccione...</option>
                                <option value="Subterráneo" {{ old('tipo_derecho_riego', $propiedad->tipo_derecho_riego) == 'subterráneo' ? 'selected' : '' }}>
                                    Subterráneo
                                </option>
                                <option value="Superficial" {{ old('tipo_derecho_riego', $propiedad->tipo_derecho_riego) == 'superficial' ? 'selected' : '' }}>
                                    Superficial
                                </option>
                                <option value="Ambos" {{ old('tipo_derecho_riego', $propiedad->tipo_derecho_riego) == 'ambos' ? 'selected' : '' }}>
                                    Ambos
                                </option>
                            </select>
                        </div>
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center">
                            <input type="checkbox" name="rut" id="rut" class="mr-2 custom-checkbox"
                                   {{ old('rut', $propiedad->rut) ? 'checked' : '' }} onchange="document.getElementById('rutFields').style.display = this.checked ? 'block' : 'none';">
                        ...
                        <div id="rutFields" style="display:none;" class="ml-7 space-y-4">
                            <div>
                                ...
                                <input id="rut_valor" name="rut_valor" type="number" step="1"
                                       class="w-full p-2 border border-gray-300 rounded"
                                       value="{{ old('rut_valor', isset($propiedad) ? $propiedad->rut_valor : '') }}">
                            </div>
                            <div>
                                <label class="block text-gray-700 font-semibold mb-1" for="rut_archivo_file">Adjuntar
                                    RUT (PDF, opcional)</label>
                                <input id="rut_archivo_file" name="rut_archivo_file" type="file"
                                       accept="application/pdf" class="w-full p-2 border border-gray-300 rounded">
                                @error('rut_archivo_file')
                                <span class="text-red-600 text-sm">{{ $message }}</span>
                                @enderror
                                @if($propiedad->rut_archivo)
                                    <div class="mt-2">
                                        <a href="{{ Storage::url($propiedad->rut_archivo) }}" target="_blank"
                                           class="text-blue-600 hover:text-blue-800 underline">Ver RUT actual</a>
                                    </div>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center">
                            <input type="checkbox" name="malla" id="malla" class="mr-2 custom-checkbox"
                                   {{ old('malla', $propiedad->malla) ? 'checked' : '' }} onchange="document.getElementById('mallaFields').style.display = this.checked ? 'block' : 'none';">
                            <label for="malla">¿Tiene malla antigranizo?</label>
                        </div>
                        <div id="mallaFields" style="display:none;" class="ml-7">
                            <label class="block text-gray-700 font-semibold mb-1" for="hectareas_malla">Hectáreas con
                                malla</label>
                            <input id="hectareas_malla" name="hectareas_malla" type="number" step="0.01"
                                   class="w-full p-2 border border-gray-300 rounded"
                                   value="{{ old('hectareas_malla', isset($propiedad) ? $propiedad->hectareas_malla : '') }}">
                        </div>
                    </div>
                </div>
